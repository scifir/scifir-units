...
TEST_CASE("class coordinates_nd<T> and coordinates_nd<float>")
{
	SECTION("Constructors of coordinates_nd<> class")
	{
		...
		CHECK (bool(to_string(a5) == "(5 m,5 m,1 m)"));
		coordinates_nd<> b6(8_m);
		CHECK (bool(to_string(b6) == "(8 m)"));
		coordinates_nd<> b7(6_m,4_m);
		CHECK (bool(to_string(b7) == "(6 m,4 m)"));
		coordinates_nd<> b8(100_m,angle(90.0f));
		CHECK (bool(to_string(b8) == "(0 m,100 m)"));
		...
		CHECK (bool(to_string(b4) == "(100 m,50 m,20 m)"));
		...
		CHECK (bool(to_string(d2) == "(1 m,5 m)"));
		coordinates_nd<> d3("(1 m,45\u00B0)");
		CHECK (bool(to_string(d3) == "(0.7 m,0.7 m)"));
		coordinates_nd<> d4("(1 m,5 m,2 m)");
		CHECK (bool(to_string(d4) == "(1 m,5 m,2 m)"));
	...
	SECTION("Constructors of coordinates_nd<float> class")
	{
		...
		CHECK (bool(to_string(b4) == "(100,50,20)"));
		//coordinates_nd<> b4(angle(0),angle(0),10_m);
		//CHECK (bool(to_string(b4) == "(0 m,0 m,100 m)"));
		...
		CHECK (bool(to_string(d2) == "(1,5)"));
		coordinates_nd<float> d3("(1,45\u00B0)");
		CHECK (bool(to_string(d3) == "(0.7,0.7)"));
		coordinates_nd<float> d4("(1,5,2)");
		CHECK (bool(to_string(d4) == "(1,5,2)"));
	...
	SECTION("Values of different coordinates systems of coordinates_nd<float> class")
	{
		...
		CHECK (bool(c.get_theta().display() == "29.74\u00B0"));
		CHECK (bool(std::fabs(c.get_r() - 8.3f) < 0.01f));
		CHECK (bool(c.get_phi().display() == "76.06\u00B0"));
	...
	SECTION("set_position(), rotate() and move() of coordinates_nd<> class")
	{
		...
		CHECK (bool(to_string(b2) == "(5.17 m,19.31 m)"));
	...
	SECTION("set_position(), rotate() and move() of coordinates_nd<float> class")
	{
		...
		b2.rotate_in_2d(angle(15.0f));
		CHECK (bool(to_string(b2) == "(5.17,19.31)"));
	...
	SECTION("Display of coordinates_nd<> class")
	{
		...
		CHECK (bool(a.display_cartesian_3d() == "(2 m,3 m,5 m)"));
		...
		CHECK (bool(b.display_cartesian_2d() == "(4.33 m,2.5 m)"));
		CHECK (bool(b.display_polar() == "(5 m,30\u00B0)"));
	...
	SECTION("Display of coordinates_nd<float> class")
	{
		...
		CHECK (bool(a.display_cartesian_2d() == "[no-2d]"));
		CHECK (bool(a.display_polar() == "[no-2d]"));
		CHECK (bool(a.display_cartesian_3d() == "(2,3,5)"));
		...
		CHECK (bool(b.display_cartesian_2d() == "(4.33,2.5)"));
		CHECK (bool(b.display_polar() == "(5,30\u00B0)"));
		CHECK (bool(b.display_cartesian_3d() == "[no-3d]"));
		CHECK (bool(b.display_cylindrical() == "[no-3d]"));
		CHECK (bool(b.display_spherical() == "[no-3d]"));
