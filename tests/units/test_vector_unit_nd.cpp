...
TEST_CASE("class vector_unit_nd")
{
	...
	SECTION("Assignments of vector_unit_nd classes")
	{
		...
		a = b;
		CHECK (bool(to_string(a) == "100 g 50° 20° 20°"));
		...
		scalar_unit f("100 g");
		e = f;
		CHECK (bool(to_string(e) == "100 g 50° 20°"));
		vector_unit_nd g("10 g",{50.0f,20.0f});
		scalar_unit h("100 g");
		g = std::move(h);
		CHECK (bool(to_string(g) == "100 g 50° 20°"));
	}
	...
	SECTION("point_to() function of vector_unit_nd classes for the 2d case")
	{
		...
		a.point_to(direction::LEFT);
		CHECK (bool(a.angles[0].get_value() == 180.0f));
		a.point_to(direction::RIGHT);
		CHECK (bool(a.angles[0].get_value() == 0.0f));
		a.point_to(direction::TOP);
		CHECK (bool(a.angles[0].get_value() == 90.0f));
		a.point_to(direction::BOTTOM);
		CHECK (bool(a.angles[0].get_value() == 270.0f));
		a.point_to(direction::LEFT_TOP);
		CHECK (bool(a.angles[0].get_value() == 135.0f));
		a.point_to(direction::RIGHT_TOP);
		CHECK (bool(a.angles[0].get_value() == 45.0f));
		a.point_to(direction::RIGHT_BOTTOM);
		CHECK (bool(a.angles[0].get_value() == 315.0f));
		a.point_to(direction::LEFT_BOTTOM);
		CHECK (bool(a.angles[0].get_value() == 225.0f));
	}
	...
	SECTION("point_to() function of vector_unit_nd classes for the 3d case")
	{
		...
		a.point_to(direction::LEFT);
		CHECK (bool(a.angles[0].get_value() == 270.0f));
		CHECK (bool(a.angles[1].get_value() == 90.0f));
		a.point_to(direction::RIGHT);
		CHECK (bool(a.angles[0].get_value() == 90.0f));
		CHECK (bool(a.angles[1].get_value() == 90.0f));
		a.point_to(direction::TOP);
		CHECK (bool(a.angles[0].get_value() == 0.0f));
		CHECK (bool(a.angles[1].get_value() == 0.0f));
		a.point_to(direction::BOTTOM);
		CHECK (bool(a.angles[0].get_value() == 0.0f));
		CHECK (bool(a.angles[1].get_value() == 180.0f));
		a.point_to(direction::LEFT_TOP);
		CHECK (bool(a.angles[0].get_value() == 270.0f));
		CHECK (bool(a.angles[1].get_value() == 45.0f));
		a.point_to(direction::RIGHT_TOP);
		CHECK (bool(a.angles[0].get_value() == 90.0f));
		CHECK (bool(a.angles[1].get_value() == 45.0f));
		a.point_to(direction::RIGHT_BOTTOM);
		CHECK (bool(a.angles[0].get_value() == 90.0f));
		CHECK (bool(a.angles[1].get_value() == 135.0f));
		a.point_to(direction::LEFT_BOTTOM);
		CHECK (bool(a.angles[0].get_value() == 270.0f));
		CHECK (bool(a.angles[1].get_value() == 135.0f));
	...
	SECTION("Operators of vector_unit_nd classes with scalar_unit classes")
	{
		...
		scalar_unit a2("5 g");
		CHECK (bool(to_string(a * a2) == "50 g*kg*m/s2 10° 10°"));
		vector_unit_nd a_2("10 N",vector<float>{10.0f});
		CHECK (bool(to_string(a_2 * a2) == "50 g*kg*m/s2 10°"));
		vector_unit_nd a_3("10 N");
		CHECK (bool(to_string(a_3 * a2) == "50 g*kg*m/s2"));
		vector_unit_nd a_4("10 N",{10.0f,10.0f,20.0f});
		CHECK (bool(to_string(a_4 * a2) == "50 g*kg*m/s2 10° 10° 20°"));
		vector_unit_nd a3 = (a * a2);
		a3.change_dimensions("N*g");
		CHECK (bool(to_string(a3) == "50 N*g 10° 10°"));
		CHECK (bool(to_string(a2 * a) == "50 g*kg*m/s2 10° 10°"));
		CHECK (bool(to_string(a2 * a_2) == "50 g*kg*m/s2 10°"));
		CHECK (bool(to_string(a2 * a_3) == "50 g*kg*m/s2"));
		CHECK (bool(to_string(a2 * a_4) == "50 g*kg*m/s2 10° 10° 20°"));
		vector_unit_nd b("10 N",{10.0f,10.0f});
		scalar_unit b2("5 g");
		CHECK (bool(to_string(b / b2) == "2000 m/s2 10° 10°"));
		vector_unit_nd b_2("10 N",vector<float>{10.0f});
		CHECK (bool(to_string(b_2 / b2) == "2000 m/s2 10°"));
		vector_unit_nd b_3("10 N");
		CHECK (bool(to_string(b_3 / b2) == "2000 m/s2"));
		vector_unit_nd b_4("10 N",{10.0f,10.0f,20.0f});
		CHECK (bool(to_string(b_4 / b2) == "2000 m/s2 10° 10° 20°"));
		vector_unit_nd c("10 N",{10.0f,10.0f});
		scalar_unit c2(10_N / 5_N);
		CHECK (bool(to_string(c ^ c2) == "100 N2 10° 10°"));
		vector_unit_nd c_2("10 N",vector<float>{10.0f});
		CHECK (bool(to_string(c_2 ^ c2) == "100 N2 10°"));
		vector_unit_nd c_3("10 N");
		CHECK (bool(to_string(c_3 ^ c2) == "100 N2"));
		vector_unit_nd c_4("10 N",{10.0f,10.0f,20.0f});
		CHECK (bool(to_string(c_4 ^ c2) == "100 N2 10° 10° 20°"));
	...
	SECTION("Numeric operators of vector_unit_nd classes")
	{
		...
		CHECK (bool(to_string(a + 10.0f) == "20 N 20° 20°"));
		CHECK (bool(to_string(10.0f + a) == "20 N 20° 20°"));
		vector_unit_nd b("10 N",{20.0f,20.0f});
		CHECK (bool(to_string(b - 5.0f) == "5 N 20° 20°"));
		CHECK (bool(to_string(20.0f - b) == "10 N 20° 20°"));
		vector_unit_nd c("10 N",{20.0f,20.0f});
		CHECK (bool(to_string(c * 2.0f) == "20 N 20° 20°"));
		CHECK (bool(to_string(2.0f * c) == "20 N 20° 20°"));
		vector_unit_nd d("10 N",{20.0f,20.0f});
		CHECK (bool(to_string(d / 2.0f) == "5 N 20° 20°"));
		CHECK (bool(to_string(20.0f / d) == "2 1/N 20° 20°"));
		vector_unit_nd e("10 N",{20.0f,20.0f});
		CHECK (bool(to_string(e ^ 2) == "100 N2 20° 20°"));
		vector_unit_nd f("10 N",{20.0f,20.0f});
		f += 5.0f;
		CHECK (bool(to_string(f) == "15 N 20° 20°"));
		vector_unit_nd g("10 N",{20.0f,20.0f});
		g -= 5.0f;
		CHECK (bool(to_string(g) == "5 N 20° 20°"));
		vector_unit_nd h("10 N",{20.0f,20.0f});
		h *= 2.0f;
		CHECK (bool(to_string(h) == "20 N 20° 20°"));
		vector_unit_nd i("10 N",{20.0f,20.0f});
		i *= -2.0f;
		CHECK (bool(to_string(i) == "20 N 200° 160°"));
		vector_unit_nd j("10 N",{20.0f,20.0f});
		j /= 2.0f;
		CHECK (bool(to_string(j) == "5 N 20° 20°"));
		vector_unit_nd k("10 N",{20.0f,20.0f});
		k /= -2.0f;
		CHECK (bool(to_string(k) == "5 N 200° 160°"));
	}

	SECTION("Projections of vector_unit_nd classes")
	{
		vector_unit_nd a("2 N",{10.0f,10.0f});
		CHECK (bool(a.x_projection().display(2) == "0.34 N"));
		CHECK (bool(a.y_projection().display(2) == "0.06 N"));
		CHECK (bool(a.z_projection().display(2) == "1.96 N"));
		CHECK (bool(a.n_projection(1).display(2) == "0.34 N"));
		CHECK (bool(a.n_projection(2).display(2) == "0.06 N"));
		CHECK (bool(a.n_projection(3).display(2) == "1.96 N"));
		CHECK (bool(a.n_projection(4).display(2) == "0 [empty]"));
		vector_unit_nd b("2 N",vector<float>{10.0f});
		CHECK (bool(b.x_projection().display(2) == "1.96 N"));
		CHECK (bool(b.y_projection().display(2) == "0.34 N"));
		CHECK (bool(b.z_projection().display(2) == "0 [empty]"));
		vector_unit_nd c("2 N");
		CHECK (bool(c.x_projection().display(2) == "2 N"));
		CHECK (bool(c.y_projection().display(2) == "0 [empty]"));
		CHECK (bool(c.z_projection().display(2) == "0 [empty]"));
		vector_unit_nd d("2 N",{10.0f,10.0f,20.0f});
		CHECK (bool(d.x_projection().display(2) == "0 [empty]"));
		CHECK (bool(d.y_projection().display(2) == "0 [empty]"));
		CHECK (bool(d.z_projection().display(2) == "0 [empty]"));
	}
	...
	SECTION("Invertion of vector_unit_nd classes")
	{
		vector_unit_nd a("2 N",{10.0f,10.0f});
		a.invert();
		CHECK (bool(to_string(a) == "2 N 190° 170°"));
		vector_unit_nd b("2 N",vector<float>{10.0f});
		b.invert();
		CHECK (bool(to_string(b) == "2 N 190°"));
	}

	SECTION("Display of vector_unit_nd classes")
	{
		vector_unit_nd a = vector_unit_nd(1,"N",{20,20,20,20});
		CHECK (bool(a.vectorial_display() == "1 N 20° 20° 20° 20°"));
		CHECK (bool(a.vectorial_derived_display() == "1 kg*m/s2 20° 20° 20° 20°"));
		CHECK (bool(a.vectorial_custom_display("g*m/s2") == "1000 g*m/s2 20° 20° 20° 20°"));
	}
	...
	SECTION("Math functions of vector_unit_nd classes")
	{
		...
		CHECK (bool(scifir::same_nd(g,g2) == true));
		CHECK (bool(scifir::same_direction(g,g2) == true));
		CHECK (bool(scifir::parallel(g,g2) == true));
		vector_unit_nd g3("2 N",{10.0f,10.0f});
		vector_unit_nd g4("1 m",{10.0f,20.0f});
		CHECK (bool(scifir::same_nd(g3,g4) == true));
		CHECK (bool(scifir::same_direction(g3,g4) == false));
		CHECK (bool(scifir::parallel(g3,g4) == false));
		vector_unit_nd g5("2 N",vector<float>{10.0f});
		vector_unit_nd g6("1 m",{10.0f,20.0f});
		CHECK (bool(scifir::same_nd(g5,g6) == false));
		CHECK (bool(scifir::parallel(g5,g6) == false));
		CHECK (bool(scifir::orthogonal(g5,g6) == false));
		...
		CHECK (bool(scifir::parallel(g7,g8) == true));
		CHECK (bool(scifir::orthogonal(g7,g8) == false));
		CHECK (bool(scifir::parallel(g7,g9) == false));
		CHECK (bool(scifir::orthogonal(g7,g9) == true));
		vector_unit_nd h("2 N",{0.0f,0.0f});
		vector_unit_nd h2("1 m",{0.0f,90.0f});
		CHECK (bool(scifir::orthogonal(h,h2) == true));
		vector_unit_nd i("2 N");
		vector_unit_nd i2("1 m");
		CHECK (bool(scifir::parallel(i,i2) == true));
		CHECK (bool(scifir::orthogonal(i,i2) == false));
	}
	...
	SECTION("String operations with vector_unit_nd classes")
	{
		...
		string b2 = "b: ";
		b2 += b;
		CHECK (bool("b: 2 N 10° 10°"));
		vector_unit_nd c("2 N",{10.0f,10.0f});
		CHECK (bool(("b: " + c) == "b: 2 N 10° 10°"));
		CHECK (bool((c + " b") == "2 N 10° 10° b"));
	}
