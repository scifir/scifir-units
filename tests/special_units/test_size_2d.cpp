...
TEST_CASE("class size_2d<T> and size_2d<float>") {
	SECTION("Constructors of size_2d<> class")
	{
		...
		size_2d<> b2(2_m,3_m);
		size_2d<> b(b2);
		CHECK (bool(to_string(b) == "2 m * 3 m"));
		size_2d<> c2(2_m,3_m);
		size_2d<> c(std::move(c2));
		CHECK (bool(to_string(c) == "2 m * 3 m"));
		size_2d d(1_m,1_m);
		CHECK (bool(to_string(d) == "1 m * 1 m"));
		size_2d<> e("1 m","1 m");
		CHECK (bool(to_string(e) == "1 m * 1 m"));
		size_2d<> f("1 m * 1 m");
		CHECK (bool(to_string(f) == "1 m * 1 m"));
	}
	...
	SECTION("Constructors of size_2d<float> class")
	{
		...
		size_2d<float> b(b2);
		CHECK (bool(to_string(b) == "2 * 3"));
		size_2d<float> c2(2.0f,3.0f);
		size_2d<float> c(std::move(c2));
		CHECK (bool(to_string(c) == "2 * 3"));
		size_2d d(1.0f,1.0f);
		CHECK (bool(to_string(d) == "1 * 1"));
		size_2d<float> e("1","1");
		CHECK (bool(to_string(e) == "1 * 1"));
		size_2d<float> f("1 * 1");
		CHECK (bool(to_string(f) == "1 * 1"));
	}
	...
	SECTION("Assignments of size_2d<> class")
	{
		...
		size_2d<> a2(2_m,3_m);
		a = a2;
		CHECK (bool(to_string(a) == "2 m * 3 m"));
		size_2d<> b;
		size_2d<> b2(2_m,3_m);
		b = std::move(b2);
		CHECK (bool(to_string(b) == "2 m * 3 m"));
	...
	SECTION("Operators of size_2d<> class")
	{
		size_2d<> a(2_m,5_m);
		size_2d<> b(3_m,1_m);
		CHECK (bool(to_string(a + b) == "5 m * 6 m"));
		size_2d<> c(2_m,5_m);
		size_2d<> d(1_m,2_m);
		CHECK (bool(to_string(c - d) == "1 m * 3 m"));
		size_2d<> e(2_m,5_m);
		size_2d<> f(1_m,1_m);
		e += f;
		CHECK (bool(to_string(e) == "3 m * 6 m"));
		size_2d<> g(2_m,5_m);
		size_2d<> h(1_m,1_m);
		g -= h;
		CHECK (bool(to_string(g) == "1 m * 4 m"));
	}
	...
	SECTION("String operators of size_2d<> class")
	{
		...
		string a = "size_2d ";
		a += size_2d<>("2 m * 3 m");
		CHECK (bool(a == "size_2d 2 m * 3 m"));
		size_2d<> b = size_2d<>("2 m * 3 m");
		CHECK (bool("b " + b == "b 2 m * 3 m"));
		CHECK (bool(b + " b" == "2 m * 3 m b"));
	}
	...
	SECTION("String operators of size_2d<float> class")
	{
		...
		string a = "size_2d ";
		a += size_2d<float>("2 * 3");
		CHECK (bool(a == "size_2d 2 * 3"));
		size_2d<float> b = size_2d<float>("2 * 3");
		CHECK (bool("b " + b == "b 2 * 3"));
		CHECK (bool(b + " b" == "2 * 3 b"));
	}
	...
	SECTION("Streams of size_2d<> class")
	{
		ostringstream a;
        a << size_2d<>(2_m,3_m);
        CHECK (bool(a.str() == "2 m * 3 m"));
        stringstream b;
        b << "1 m * 3 m";
        size_2d<> b2;
        b >> b2;
        CHECK (bool(to_string(b2) == "1 m * 3 m"));
	}
	...
	SECTION("Streams of size_2d<float> class")
	{
        ...
        CHECK (bool(a.str() == "2 * 3"));
        stringstream b;
        b << "1 * 3";
        size_2d<float> b2;
        b >> b2;
        CHECK (bool(to_string(b2) == "1 * 3"));
	}
}