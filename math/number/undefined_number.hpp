...
namespace math::number
{
	...
	class undefined_number
	{
		public:
			...
			undefined_number(math::topology::space_type new_value)
			{
				value = new_value;
				error_code = 0;
			}
			...
			const math::topology::space_type& get_value() const
			{
				return value;
			}
		...
		protected:
			int error_code;
			math::topology::space_type value;
	};

	template<typename T>
	math::topology::space_type abs(const undefined_number<T>& x)
	{
...
U operator +(V x, const math::number::undefined_number<U>& y)
{
...
bool operator ==(math::number::undefined_number<T> x, math::number::undefined_number<U> y)
{
...
bool operator ==(const math::number::undefined_number<T>& x, U y)
{
	if(x.is_defined())
	{
		if(x.get_value() == y)
...
template<typename T>
void operator +=(wstring& x, const math::number::undefined_number<T>& y)
{
	wostringstream output;
	output << y;
	x += output.str();
...
template<typename T>
wstring operator +(const wstring& x, const math::number::undefined_number<T>& y)
{
	wostringstream output;
	output << x;
	output << y;
...
template<typename T>
wstring operator +(const math::number::undefined_number<T>& y, const wstring& x)
{
	wostringstream output;
	output << y;
	output << x;
...
template<typename T>
wostream & operator <<(wostream& os, const math::number::undefined_number<T>& x)
{
	return os << x.print();
}
