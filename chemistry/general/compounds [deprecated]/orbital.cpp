...
namespace chemistry
{
	orbital::orbital(orbital_symbol new_orbital_specie, int new_period, int electron_number)
	{
		...
		period = new_period;
		electrons = vector<physics::particles::electron>();
		if(electron_number >= 1)
		{
			physics::particles::electron electron1 = physics::particles::electron(-0.5);
			electrons.push_back(electron1);
			if(electron_number == 2)
			{
				physics::particles::electron electron2 = physics::particles::electron(0.5);
...
ostream& operator <<(ostream& os, chemistry::orbital_symbol orbital)
{
	ostringstream output;
